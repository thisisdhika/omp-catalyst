...
import { readActiveMode, applyModeToFile } from "../mode.js";
...
export async function initCommand() {
  ...
  // Apply active mode to freshly installed agents only
  const activeMode = readActiveMode();
  if (activeMode) {
    let changed = 0;
    for (const file of selectedAgents || []) {
      const dest = join(agentDir, "agents", file);
      if (applyModeToFile(dest, file.replace(".md", ""), activeMode)) {
        changed++;
      }
    }
    if (changed > 0) {
      console.log(chalk.dim(`  Applied model mode: ${activeMode}`));
    }
  }
